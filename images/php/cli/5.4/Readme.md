...
Example : 
   ...
   ``` sh
        docker run --user=$(id -u):$(id -g) lethak/php_cli:5.4
   ```
...
and then define the SSH_AUTH_SOCK environment variable inside the container with the value of the destination path :     
   ...
   ``` sh
       docker run --user=$(id -u):$(id -g) -v "${SSH_AUTH_SOCK}:/ssh-agent:ro" -e "SSH_AUTH_SOCK=/ssh-agent" lethak/php_cli:5.4
   ```
